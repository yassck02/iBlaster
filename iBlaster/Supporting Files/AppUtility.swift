...
//    }
    ...
    static var strokeShader: SKShader!
    static var fillShader: SKShader!
    
    static func createShaders() {
        strokeShader = SKShader(source: "" +
            "void main() {" +
            "gl_FragColor = float4(1.0, 1.0, 1.0, 1.0);" +
        "}")
        
        fillShader = SKShader(source: "" +
            "void main() {" +
            "gl_FragColor = float4(gl_FragCoord.x/screen_size.x, 1.0, 1-(gl_FragCoord.y/screen_size.y), 1.0);" +
        "}")
        
        fillShader.uniforms = [
            SKUniform(name: "screen_size", vectorFloat2: vector_float2(Float(UIScreen.main.bounds.width),
